    ...
    if the_list:
        for i in the_list:
            if name == i['title_romaji'].lower() or name == i['title_english'].lower() or name == i['title_japanese']:
                ...
                clean_name = i['title_romaji'].replace('/', '')
                url = url + clean_name.replace(' ', '')
                return url
    return None
...
def splitted_search(names, path, a_object):
    ...
    if the_list:
        for content in the_list:
            for name in names:
                if name not in content['title_romaji'].lower() and name not in content[
                    'title_english'].lower() and name not in content['title_japanese']:
                    break
                if name == names[len(names) - 1]:
                    ...
                    clean_name = content['title_romaji'].replace('/', '')
                    url = url + clean_name.replace(' ', '')
                    return url
    return None
